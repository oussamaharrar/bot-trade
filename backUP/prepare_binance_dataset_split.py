...
def process_tick_file(file_path: str) -> pd.DataFrame:
    ...
    df['rolling_mean_10'] = df['price'].rolling(window=10).mean().bfill()
    df['rolling_std_10'] = df['price'].rolling(window=10).std().bfill()
...
def deep_main():
    ...
    for f in tqdm(files, desc="Deep RL Data Prep"):
        ...
        try:
            ...
            num_chunks = (len(df_processed) // chunk_size) + 1
            for i in range(num_chunks):
                chunk = df_processed.iloc[i*chunk_size : (i+1)*chunk_size]
                if chunk.empty:
                    continue
                chunk_date = chunk.index[0].strftime("%Y-%m-%d")
                chunk_symbol = chunk['symbol'].iloc[0]
                chunk_name = f"{chunk_symbol}_{chunk_date}_part{i+1:02d}.feather"
                out_path = os.path.join(OUTPUT_DIR, chunk_name)
                chunk.reset_index().to_feather(out_path)

                try:
                    with open("memory/memory.json", "r", encoding="utf-8") as memf:
                        memory = json.load(memf)
                except:
                    memory = {}

                memory.setdefault("processed_data", {})
                memory["processed_data"][chunk_name] = {
                    "symbol": chunk_symbol,
                    "date": chunk_date,
                    "status": "processed"
                }
                with open("memory/memory.json", "w", encoding="utf-8") as memf:
                    json.dump(memory, memf, indent=2)
